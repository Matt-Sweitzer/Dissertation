newNetworkTieDecision3 = stepFactory.createStep("newNetworkTieDecision3")

newNetworkTieDecision3.run = {
  //Add record of step start
  println("newNetworkTieDecision3.run -- Counter: " + counter)
  a.addEvent("StepStart", ["step":"newNetworkTieDecision3", "counter":counter])
        ...
        if(v1.bot == true && !v1.updatedThisRound.contains(v2.number)){
          ...
          if(v2.updatedThisRound.contains(v1.number) || v2.bot == true){
            ...
            if(!v1.curNeighbors.contains(v2.number) && !v1.alterHist.contains(v2.number)){// new, no history
              a.addEvent("SelectedForTieUpdate", ["round":"3", "ego.number":v1.number, "alter.number":v2.number, "status":"New - no history"])
              //Record bot ignored
                a.addEvent("BotTieIgnored", ["round.number":3, "ego.number":v1.number, "alter.number":v2.number])
            ...
            if(!v1.curNeighbors.contains(v2.number) && v1.alterHist.contains(v2.number)){// new, have history
              a.addEvent("SelectedForTieUpdate", ["round":"3", "ego.number":v1.number, "alter.number":v2.number, "status":"New - have history"])
              //Record bot ignored
                a.addEvent("BotTieIgnored", ["round.number":3, "ego.number":v1.number, "alter.number":v2.number])
                println("Bot #" + v1.number + " ignored a tie with player #" + v2.number)
                ignoreCount += 1
            ...
            if(v1.curNeighbors.contains(v2.number)){// existing tie
              a.addEvent("SelectedForTieUpdate", ["round":"3", "ego.number":v1.number, "alter.number":v2.number, "status":"Existing tie"])
              //Record bot maintained
                a.addEvent("BotTieMaintained", ["round.number":3, "ego.number":v1.number, "alter.number":v2.number])
          ...
          if(!v2.updatedThisRound.contains(v1.number) && v2.bot == false){
            ...
            if(!v2.curNeighbors.contains(v1.number) && !v2.alterHist.contains(v1.number)){// new, no history
              a.addEvent("BotDeferred", ["round":"3", "bot.number":v1.number, "player.number":v2.number, "status":"New - no history"])
            ...
            if(!v2.curNeighbors.contains(v1.number) && v2.alterHist.contains(v1.number)){// new, have history
              a.addEvent("BotDeferred", ["round":"3", "bot.number":v1.number, "player.number":v2.number, "status":"New - have history"])
            ...
            if(v2.curNeighbors.contains(v1.number)){// existing tie
              a.addEvent("BotDeferred", ["round":"3", "bot.number":v1.number, "player.number":v2.number, "status":"Existing tie"])
  ...
  g.V.filter{(it.goodplayer || it.ai==1) && it.updatedThisRound.size() > 0}.each{v1->
    ...
    if(!v1.curNeighbors.contains(v1.updatedThisRound[0]) && !v1.alterHist.contains(v1.updatedThisRound[0])){//new, no history
      choiceNewTie_Rnd3(v1.number, v1.updatedThisRound[0])
    }
    if(!v1.curNeighbors.contains(v1.updatedThisRound[0]) && v1.alterHist.contains(v1.updatedThisRound[0])){//new, have history
      choiceNewTie_wHist_Rnd3(v1.number, v1.updatedThisRound[0])
    }
    if(v1.curNeighbors.contains(v1.updatedThisRound[0])){//existing tie
      choiceExistingTie_Rnd3(v1.number, v1.updatedThisRound[0])
    }
  }
}

newNetworkTieDecision3.done = {
  println("newNetworkTieDecision3.done -- Counter: " + counter)
