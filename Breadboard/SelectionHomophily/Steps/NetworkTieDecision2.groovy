networkTieDecision2 = stepFactory.createStep("NetworkTieDecision2")

networkTieDecision2.run = {
  println "networkTieDecision2.run -- Counter: " + counter
  a.addEvent("StepStart", ["step":"networkTieDecision2", "counter":counter])
          ...
          if(g.hasEdge(v1, v2) == true){
            ...
            if(v1.bot == false && v1.ai == null){
              ...
              if(uncertainty){
                ...
                if(v1.History_Rnd2[histIndex] > 0 && v2.History_Rnd2[histIndex] < 8){histLength += 1}
                v1.text = c.get("Payment", String.format("%.2f", (v1.payment))) + c.get("Network_Tie_Uncertain_Head", "2", v2.number, histLength)
                ...
                qtext2 = "How confident are you in your assessment of <strong>" + v2.number.toString() + "</strong>’s opinion on the topic of <strong>" + getTopic(Topic3_Q) + "</strong>?"
                qTopics = []
                qAAtt = []
                qEAtt = []
          ...
          if(g.hasEdge(v1, v2) == false){
            ...
            if(!v1.alterHist.contains(v2.number)){ //new, no history
              ...
              if(v1.bot == false && v1.ai == null){
                ...
                if(uncertainty){
                  v1.text = c.get("Payment", String.format("%.2f", (v1.payment))) + c.get("Network_Tie_Uncertain_Head", "2", v2.number, "0")
                  v1.text += """<audio autoplay src="https://raw.githubusercontent.com/Matt-Sweitzer/matt-sweitzer.github.io/master/misc/beep-09.mp3"></audio>""".toString()
                  qtext = "In the next round, the topic of discussion will be <strong>" + getQText(Topic3_Q, Topic3_A) + "</strong>. We are interested in what you might think <strong>" + v2.number.toString() + "</strong> would say about this topic. <br> <br> <br> If you had to guess what <strong>" + v2.number.toString() + "</strong> thinks about this topic, where would you place them on the following scale? Consider <em>only their opinion</em> and not whether you agree with them."
                  qtext2 = "How confident are you in your assessment of <strong>" + v2.number.toString() + "</strong>’s opinion on the topic of <strong>" + getTopic(Topic3_Q) + "</strong>?"
                  qUncer_Proj = [id: "qUncer_Proj",
...
networkTieDecision2.done = {
  println "networkTieDecision2.done -- Counter: " + counter
  if(counter < choiceCount){
    networkTieDecision2.start()
  }
