networkDisclosure3 = stepFactory.createStep("NetworkDisclosure3")

networkDisclosure3.run = {
 println "networkDisclosure3.run"
 a.addEvent("StepStart", ["step":"networkDisclosure3"])
 ...
 g.V.filter{it.goodplayer || it.ai==1}.each{v->
  if(condition==2){
    v.Disclose_3 = v.curNeighbors
  }
 }

 if(condition==2){
   g.V.filter{(it.goodplayer && it.bot==false) || it.ai==1}.each{ v->
     v.text += c.get("Network_Disclose_Head", "3", getTopic(Topic3_Q))
   }
   ...
   g.V.filter{it.goodplayer && it.bot==false && it.curNeighbors.size()>0}.each{v->
     ...
     a.add(v, createSurveyQuestion(qDisclose_3, v))
   }
 }
}

networkDisclosure3.done = {
  println "networkDisclosure3.done"
  networkDiscloseRecord3.start()
}
