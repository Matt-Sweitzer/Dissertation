networkTieDecision10 = stepFactory.createStep("NetworkTieDecision10")

networkTieDecision10.run = {
  println "networkTieDecision10.run -- Counter: " + counter
  a.addEvent("StepStart", ["step":"networkTieDecision10", "counter":counter])

  g.V.filter{(it.goodplayer && it.bot==false) || it.ai==1}.each{ v->
    v.text = c.get("Payment", String.format("%.2f", (v.payment))) + c.get("Network_Tie_Wait", "10")
  }
          ...
          if(g.hasEdge(v1, v2) == true){
            ...
            if(v1.bot == false && v1.ai == null){
              ...
              if(uncertainty){
                ...
                if(v1.History_Rnd8[histIndex] > 0 && v1.History_Rnd8[histIndex] < 8){histLength += 1}
                if(v1.History_Rnd9[histIndex] > 0 && v1.History_Rnd9[histIndex] < 8){histLength += 1}
                if(v1.History_Rnd10[histIndex] > 0 && v1.History_Rnd10[histIndex] < 8){histLength += 1}
                v1.text = c.get("Payment", String.format("%.2f", (v1.payment))) + c.get("Network_Tie_Uncertain_Head", "10", v2.number, histLength)
                ...
                qtext2 = "How confident are you in your assessment of <strong>" + v2.number.toString() + "</strong>’s opinion on the topic of <strong>" + getTopic(Topic_HeldOut_Q) + "</strong>?"
                qTopics = []
                qAAtt = []
                qEAtt = []
                ...
                if(v1.History_Rnd8[histIndex] != 0){
                  qTopics << getTopic(Topic8_Q)
                  qAAtt << attConvert(v1.History_Rnd8[histIndex])
                  qEAtt << attConvert(getAttitudeN(Topic8_Q, Topic8_A, v1).toInteger())
                }
                if(v1.History_Rnd9[histIndex] != 0){
                  qTopics << getTopic(Topic9_Q)
                  qAAtt << attConvert(v1.History_Rnd9[histIndex])
                  qEAtt << attConvert(getAttitudeN(Topic9_Q, Topic9_A, v1).toInteger())
                }
                if(v1.History_Rnd10[histIndex] != 0){
                  qTopics << getTopic(Topic10_Q)
                  qAAtt << attConvert(v1.History_Rnd10[histIndex])
              ...
              if(! uncertainty){
                ...
                if(v1.History_Rnd8[histIndex] != 0){
                  rowCount += 1
                  if(rowCount == 1){
                    v1.text += c.get("Network_Tie_Table_1", getTopic(Topic8_Q), attConvert(v1.History_Rnd8[histIndex]), attConvert(getAttitudeN(Topic8_Q, Topic8_A, v1).toInteger()))
                    rowCount += 1
                  }
                  if(rowCount == 3){
                    v1.text += c.get("Network_Tie_Table_2", getTopic(Topic8_Q), attConvert(v1.History_Rnd8[histIndex]), attConvert(getAttitudeN(Topic8_Q, Topic8_A, v1).toInteger()))
                    rowCount = 0
                  }
                }
                if(v1.History_Rnd9[histIndex] != 0){
                  rowCount += 1
                  if(rowCount == 1){
                    v1.text += c.get("Network_Tie_Table_1", getTopic(Topic9_Q), attConvert(v1.History_Rnd9[histIndex]), attConvert(getAttitudeN(Topic9_Q, Topic9_A, v1).toInteger()))
                    rowCount += 1
                  }
                  if(rowCount == 3){
                    v1.text += c.get("Network_Tie_Table_2", getTopic(Topic9_Q), attConvert(v1.History_Rnd9[histIndex]), attConvert(getAttitudeN(Topic9_Q, Topic9_A, v1).toInteger()))
                    rowCount = 0
                  }
                }
                if(v1.History_Rnd10[histIndex] != 0){
                  rowCount += 1
                  if(rowCount == 1){
                    v1.text += c.get("Network_Tie_Table_1", getTopic(Topic10_Q), attConvert(v1.History_Rnd10[histIndex]), attConvert(getAttitudeN(Topic10_Q, Topic10_A, v1).toInteger()))
                    rowCount += 1
                  }
                  if(rowCount == 3){
                    v1.text += c.get("Network_Tie_Table_2", getTopic(Topic10_Q), attConvert(v1.History_Rnd10[histIndex]), attConvert(getAttitudeN(Topic10_Q, Topic10_A, v1).toInteger()))
                    rowCount = 0
                  }
                }
                a.add(v1, [
          ...
          if(g.hasEdge(v1, v2) == false){
            ...
            if(!v1.alterHist.contains(v2.number)){ //new, no history
              ...
              if(v1.bot == false && v1.ai == null){
                ...
                if(uncertainty){
                  v1.text = c.get("Payment", String.format("%.2f", (v1.payment))) + c.get("Network_Tie_Uncertain_Head", "10", v2.number, "0")
                  v1.text += """<audio autoplay src="https://raw.githubusercontent.com/Matt-Sweitzer/matt-sweitzer.github.io/master/misc/beep-09.mp3"></audio>""".toString()
                  qtext = "In the next round, the topic of discussion will be <strong>" + getQText(Topic_HeldOut_Q, Topic_HeldOut_A) + "</strong>. We are interested in what you might think <strong>" + v2.number.toString() + "</strong> would say about this topic. <br> <br> <br> If you had to guess what <strong>" + v2.number.toString() + "</strong> thinks about this topic, where would you place them on the following scale? Consider <em>only their opinion</em> and not whether you agree with them."
                  qtext2 = "How confident are you in your assessment of <strong>" + v2.number.toString() + "</strong>’s opinion on the topic of <strong>" + getTopic(Topic_HeldOut_Q) + "</strong>?"
                  qUncer_Proj = [id: "qUncer_Proj",
...
networkTieDecision10.done = {
  println "networkTieDecision10.done -- Counter: " + counter
  if(counter < choiceCount){
    networkTieDecision10.start()
  }
  if(counter >= choiceCount && counter < 9999){
    counter = 10000
    networkTieDone10.start()
  }
}
