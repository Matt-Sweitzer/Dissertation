networkDiscloseRecord8 = stepFactory.createStep("networkDiscloseRecord8")

networkDiscloseRecord8.run = {
  println "networkDiscloseRecord8.run"
  a.addEvent("StepStart", ["step":"networkDiscloseRecord8"])
  ...
  g.V.filter{it.goodplayer || it.ai==1}.each{v->
    ...
    for(i = 0; i < sortedList.size(); i++){
      if(v.curNeighbors.contains(sortedList[i])){
        g.V.filter{it.number == sortedList[i]}.each{n->
          if(condition==1){
            allHist << getAttitudeN(Topic8_Q, Topic8_A, n).toInteger()
          }
          if(condition==2){
            if(masterDisclose_8[i].contains(v.number)){
              allHist << getAttitudeN(Topic8_Q, Topic8_A, n).toInteger()
            }
            if(! masterDisclose_8[i].contains(v.number)){
              allHist << 99
    ...
    v.History_Rnd8 = allHist
    println "History list for v.number " + v.number + " done"
  }
}

networkDiscloseRecord8.done = {
  println "networkDiscloseRecord8.done"
  networkDiscussion8.start()
}
