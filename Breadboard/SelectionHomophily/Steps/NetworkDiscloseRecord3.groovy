networkDiscloseRecord3 = stepFactory.createStep("networkDiscloseRecord3")

networkDiscloseRecord3.run = {
  println "networkDiscloseRecord3.run"
  a.addEvent("StepStart", ["step":"networkDiscloseRecord3"])
  ...
  g.V.filter{it.goodplayer || it.ai==1}.each{v->
    ...
    for(i = 0; i < sortedList.size(); i++){
      if(v.curNeighbors.contains(sortedList[i])){
        g.V.filter{it.number == sortedList[i]}.each{n->
          if(condition==1){
            allHist << getAttitudeN(Topic3_Q, Topic3_A, n).toInteger()
          }
          if(condition==2){
            if(masterDisclose_3[i].contains(v.number)){
              allHist << getAttitudeN(Topic3_Q, Topic3_A, n).toInteger()
            }
            if(! masterDisclose_3[i].contains(v.number)){
              allHist << 99
    ...
    v.History_Rnd3 = allHist
    println "History list for v.number " + v.number + " done"
  }
}

networkDiscloseRecord3.done = {
  println "networkDiscloseRecord3.done"
  networkDiscussion3.start()
}
