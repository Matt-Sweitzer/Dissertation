newNetworkTieDecision4 = stepFactory.createStep("newNetworkTieDecision4")

newNetworkTieDecision4.run = {
  //Add record of step start
  println("newNetworkTieDecision4.run -- Counter: " + counter)
  a.addEvent("StepStart", ["step":"newNetworkTieDecision4", "counter":counter])
  ...
  g.V.filter{(it.goodplayer && it.bot==false) || it.ai==1}.each{ v->
    v.text = c.get("Payment", String.format("%.2f", (v.payment))) + c.get("Network_Tie_Wait", "4")
  }
        ...
        if(v1.bot == true && !v1.updatedThisRound.contains(v2.number)){
          ...
          if(v2.updatedThisRound.contains(v1.number) || v2.bot == true){
            ...
            if(!v1.curNeighbors.contains(v2.number) && !v1.alterHist.contains(v2.number)){// new, no history
              a.addEvent("SelectedForTieUpdate", ["round":"4", "ego.number":v1.number, "alter.number":v2.number, "status":"New - no history"])
              //Record bot ignored
                a.addEvent("BotTieIgnored", ["round.number":4, "ego.number":v1.number, "alter.number":v2.number])
            ...
            if(!v1.curNeighbors.contains(v2.number) && v1.alterHist.contains(v2.number)){// new, have history
              a.addEvent("SelectedForTieUpdate", ["round":"4", "ego.number":v1.number, "alter.number":v2.number, "status":"New - have history"])
              //Record bot ignored
                a.addEvent("BotTieIgnored", ["round.number":4, "ego.number":v1.number, "alter.number":v2.number])
                println("Bot #" + v1.number + " ignored a tie with player #" + v2.number)
                ignoreCount += 1
            ...
            if(v1.curNeighbors.contains(v2.number)){// existing tie
              a.addEvent("SelectedForTieUpdate", ["round":"4", "ego.number":v1.number, "alter.number":v2.number, "status":"Existing tie"])
              //Record bot maintained
                a.addEvent("BotTieMaintained", ["round.number":4, "ego.number":v1.number, "alter.number":v2.number])
          ...
          if(!v2.updatedThisRound.contains(v1.number) && v2.bot == false){
            ...
            if(!v2.curNeighbors.contains(v1.number) && !v2.alterHist.contains(v1.number)){// new, no history
              a.addEvent("BotDeferred", ["round":"4", "bot.number":v1.number, "player.number":v2.number, "status":"New - no history"])
            ...
            if(!v2.curNeighbors.contains(v1.number) && v2.alterHist.contains(v1.number)){// new, have history
              a.addEvent("BotDeferred", ["round":"4", "bot.number":v1.number, "player.number":v2.number, "status":"New - have history"])
            ...
            if(v2.curNeighbors.contains(v1.number)){// existing tie
              a.addEvent("BotDeferred", ["round":"4", "bot.number":v1.number, "player.number":v2.number, "status":"Existing tie"])
  ...
  g.V.filter{(it.goodplayer || it.ai==1) && it.updatedThisRound.size() > 0}.each{v1->
    ...
    if(!v1.curNeighbors.contains(v1.updatedThisRound[0]) && !v1.alterHist.contains(v1.updatedThisRound[0])){//new, no history
      choiceNewTie_Rnd4(v1.number, v1.updatedThisRound[0])
    }
    if(!v1.curNeighbors.contains(v1.updatedThisRound[0]) && v1.alterHist.contains(v1.updatedThisRound[0])){//new, have history
      choiceNewTie_wHist_Rnd4(v1.number, v1.updatedThisRound[0])
    }
    if(v1.curNeighbors.contains(v1.updatedThisRound[0])){//existing tie
      choiceExistingTie_Rnd4(v1.number, v1.updatedThisRound[0])
    }
  }
}

newNetworkTieDecision4.done = {
  println("newNetworkTieDecision4.done -- Counter: " + counter)
  networkTieDone4.start()
}
